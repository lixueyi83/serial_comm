...
SerialPort::~SerialPort()
{
    ...
    catch(...)
    {
        ...
        // But we don't want to throw within destructor, just swallow it @_@
    }
}
...
void SerialPort::Open()
{
    ...
    // 3rd parameter, mode_t mode, is optional.
...
void SerialPort::ConfigureTermios()
{
    ...
    // Set parity: https://man7.org/linux/man-pages/man3/tcflush.3.html
    switch(parity_)
    {
        case Parity::NONE:
            tty.c_cflag &= ~PARENB;
    ...
    // We used to use cfsetispeed() and cfsetospeed() with the B... macros, but this didn't allow us to
    // set custom baud rates. So now to support both standard and custom baud rates lets just make
    // everything "custom". This giant switch statement could be replaced with a map/lookup in the future.
    if (baudRateType_ == BaudRateType::STANDARD)
    {
        ...
        {
// NOTE: Just make everything "custom", and this giant table better be replaced with a map/loopup table.
#if 0 // STANDARD_BAUD_RATE
            case BaudRate::B_0:
                cfsetispeed(&tty, B0); cfsetospeed(&tty, B0);
                break;
            case BaudRate::B_50:
                cfsetispeed(&tty, B50); cfsetospeed(&tty, B50);
                break;
            case BaudRate::B_75:
                cfsetispeed(&tty, B75); cfsetospeed(&tty, B75);
                break;
            case BaudRate::B_110:
                cfsetispeed(&tty, B110); cfsetospeed(&tty, B110);
                break;
            case BaudRate::B_134:
                cfsetispeed(&tty, B134); cfsetospeed(&tty, B134);
                break;
            case BaudRate::B_150:
                cfsetispeed(&tty, B150); cfsetospeed(&tty, B150);
                break;
            case BaudRate::B_200:
                cfsetispeed(&tty, B200); cfsetospeed(&tty, B200);
                break;
            case BaudRate::B_300:
                cfsetispeed(&tty, B300); cfsetospeed(&tty, B300);
                break;
            case BaudRate::B_600:
                cfsetispeed(&tty, B600); cfsetospeed(&tty, B600);
                break;
            case BaudRate::B_1200:
                cfsetispeed(&tty, B1200); cfsetospeed(&tty, B1200);
                break;
            case BaudRate::B_1800:
                cfsetispeed(&tty, B1800); cfsetospeed(&tty, B1800);
                break;
            case BaudRate::B_2400:
                cfsetispeed(&tty, B2400); cfsetospeed(&tty, B2400);
                break;
            case BaudRate::B_4800:
                cfsetispeed(&tty, B4800); cfsetospeed(&tty, B4800);
                break;
            case BaudRate::B_9600:
                cfsetispeed(&tty, B9600); cfsetospeed(&tty, B9600);
                break;
            case BaudRate::B_19200:
                cfsetispeed(&tty, B19200); cfsetospeed(&tty, B19200);
                break;
            case BaudRate::B_38400:
                cfsetispeed(&tty, B38400); cfsetospeed(&tty, B38400);
                break;
            case BaudRate::B_57600:
                cfsetispeed(&tty, B57600); cfsetospeed(&tty, B57600);
                break;
            ...
            case BaudRate::B_460800:
                cfsetispeed(&tty, B460800); cfsetospeed(&tty, B460800);
                break;
            default:
                throw std::runtime_error(std::string() + "baudRate passed to " + __PRETTY_FUNCTION__ + " unrecognized.");
#else // CUSTOM_BAUD_RATE
            case BaudRate::B_0:
                tty.c_ispeed = 0; tty.c_ospeed = 0;
                break;
            case BaudRate::B_50:
                tty.c_ispeed = 50; tty.c_ospeed = 50;
                break;
            case BaudRate::B_75:
                tty.c_ispeed = 75; tty.c_ospeed = 75;
                break;
            case BaudRate::B_110:
                tty.c_ispeed = 110; tty.c_ospeed = 110;
                break;
            case BaudRate::B_134:
                tty.c_ispeed = 134; tty.c_ospeed = 134;
                break;
            case BaudRate::B_150:
                tty.c_ispeed = 150; tty.c_ospeed = 150;
                break;
            case BaudRate::B_200:
                tty.c_ispeed = 200; tty.c_ospeed = 200;
                break;
            case BaudRate::B_300:
                tty.c_ispeed = 300; tty.c_ospeed = 300;
                break;
            case BaudRate::B_600:
                tty.c_ispeed = 600; tty.c_ospeed = 600;
                break;
            case BaudRate::B_1200:
                tty.c_ispeed = 1200; tty.c_ospeed = 1200;
                break;
            case BaudRate::B_1800:
                tty.c_ispeed = 1800; tty.c_ospeed = 1800;
                break;
            case BaudRate::B_2400:
                tty.c_ispeed = 2400; tty.c_ospeed = 2400;
                break;
            case BaudRate::B_4800:
                tty.c_ispeed = 4800; tty.c_ospeed = 4800;
                break;
            case BaudRate::B_9600:
                tty.c_ispeed = 9600; tty.c_ospeed = 9600;
                break;
            case BaudRate::B_19200:
                tty.c_ispeed = 19200; tty.c_ospeed = 19200;
                break;
            case BaudRate::B_38400:
                tty.c_ispeed = 38400; tty.c_ospeed = 38400;
                break;
            case BaudRate::B_57600:
                tty.c_ispeed = 57600; tty.c_ospeed = 57600;
                break;
            case BaudRate::B_115200:
                tty.c_ispeed = 115200; tty.c_ospeed = 115200;
                break;
            case BaudRate::B_230400:
                tty.c_ispeed = 230400; tty.c_ospeed = 230400;
                break;
            case BaudRate::B_460800:
                tty.c_ispeed = 460800; tty.c_ospeed = 460800;
                break;
            default:
                throw std::runtime_error(std::string() + "baudRate passed to " + __PRETTY_FUNCTION__ + " unrecognized.");
#endif // BAUD_RATE switch
        }
    }
    // This does no different than STANDARD atm, but let's keep them separate for now.
    ...
    //=========================== LOCAL MODES (c_lflag) =======================//
    // Canonical input is when read waits for EOL or EOF characters before returning. In non-canonical
    // mode, the rate at which read() returns is instead controlled by c_cc[VMIN] and c_cc[VTIME]
...
void SerialPort::Read(std::string& data)
{
    ...
    // Read from file
    // We provide the underlying raw array from the readBuffer_ vector to this C api.  This will work
    // because we do not delete/resize the vector while this method is called
    // ssize_t n = read(fileDesc_, &readBuffer_[0], readBufferSize_B_);
    
    // Note: (Xueyi, 10/10/2022)
    // According to tec control communication protocol, { stx='*'(0x2A), etx='\r'(0x0D), ack='^'(0x5E) }
    //  - command:        (stx)00CCDDDDDDDDSS(etx), 16 characters.
    //  - Response(OK):   (stx)DDDDDDDDSS(etx), a successful response, 12 characters.
    //  - Response(Fail): (stx)XXXXXXXXC0(ack), when checksum is not correct, 12 characters.
    int len, index = 0;
    while(index < readBufferSize_B_)
    {
    ...
    if(len < 0)
    {
        throw std::system_error(EFAULT, std::system_category()); // Read was unsuccessful
    }

    if(len > 0)
    {
        assert(index+1 == 12);
        data = std::string(&readBuffer_[0], index+1); // Note: (Xueyi, 10/10/2022) (index+1) should always be 12.
    }
